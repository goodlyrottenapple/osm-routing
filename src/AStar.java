 ...
 * @since       2014-04-20         (the version of the package this class was first added to)
 */
public class AStar {
	...
	private Map<Container, Double> distance, heuristic_distance;
	...
	public void execute(Container source, Container target) {
		...
		heuristic_distance = new HashMap<Container, Double>();
		predecessors = new HashMap<Container, Container>();
		distance.put(source, 0.0);
		heuristic_distance.put(source, Grapher.calculateDistance(source.node.lat, source.node.lon, target.node.lat, target.node.lon));
	...
	private void findMinimalDistances(Container node) {
		...
		for (Container target : adjacentNodes) {
			if (getShortestDistance(target) > getShortestDistance(node) + getDistance(node, target)) {
                ...
                heuristic_distance.put(target, (distance.get(target) + Grapher.calculateDistance(target.node.lat, target.node.lon, this.target.node.lat, this.target.node.lon)) );		
	...
	private double getShortestHeuristicDistance(Container destination) {
		Double d = heuristic_distance.get(destination);
		if (d == null) return Double.MAX_VALUE;
		else return d;
	}
	/*
	...
	public static LinkedList<MapNode> getShortestPath(MapGraph g, Container source, Container target) {
		...
		if(g.getVertices().contains(source)) {
			...
			if(g.getVertices().contains(target)) {
				...
				LinkedList<Container> path = astar.getPath(target);
	 ...
	 */
	public static List<MapEdge> getShortestPathEdges(MapGraph g, Container source, Container target) {
		AStar astar = new AStar(g);
		...
		if(g.getVertices().contains(source)) {
			...
			System.out.println(astar.predecessors.size());
